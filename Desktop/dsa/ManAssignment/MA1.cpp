...
class Network{
	...
	public:
		...
		void deltaY(int n1, int n2, int a1, int a2){
			...
			float RA =  Ran1*Ran2/(Ran1+Ran2+R12);
			insert_to_map(n1, a1, RA);
			RA = R12*Ran1/(Ran1+Ran2+R12) + get_from_map(n1, a2);
			insert_to_map(n1, a2, RA);
			RA = R12*Ran2/(Ran1+Ran2+R12);
			insert_to_map(n2, n1, RA);				
			return;
		}
};
