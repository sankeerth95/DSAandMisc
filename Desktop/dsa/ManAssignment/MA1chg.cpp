...
void insert_to_map(int u, int v, float R){
	...
	string x = (char)(u) + "," + (char)(v);
	string y = (char)(v) + "," + (char)(u);
	...
	public:
		...
		bool reduceSeriesParallel(int start, int end){
			...
			while(sPoint != 0){	
				...
				float R = get_from_map(nbr1, v) + get_from_map(nbr2, v);

				erase_from_map(nbr1, v);
				erase_from_map(nbr2, v);
				...
				float w = get_from_map(nbr1, nbr2);
				if(w == -1){
					insertBiEdge(nbr1, nbr2, R);
				}

				else{
					insert_to_map(nbr1, nbr2, w*R/(w + R));
		...
		bool reduceWheatstone(int start, int end){
			...
			float RX = get_from_map(start, end);
			if (RX != -1){
		...
		void deltaY(int n1, int n2, int a1, int a2){
			...
			float RA = R12*Ran1/(Ran1+Ran2+R12) + get_from_map(n1, a2);
			insert_to_map(n1, a2, RA);
