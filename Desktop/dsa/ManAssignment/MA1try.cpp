...
class Network{
	...
	public:
		...
		void insertBiEdge(int u, int v, float w = -1.0f){
			...
			weight[v][u] = weight[u][v] = get_from_map(u, v);
		}
		...
		bool reduceWheatstone(int start, int end){
			...
			float RX = -1;
			if (weight[start][end] != -1){
				RX = weight[start][end];
				weight[start][end] = weight[end][start] = -1;
		...
		void deltaY(int n1, int n2, int a1, int a2){
			...
			weight[a1][n1] = weight[n1][a1] = Ran1*Ran2/(Ran1+Ran2+R12);
			weight[a2][n1] = weight[n1][a2] =  R12*Ran1/(Ran1+Ran2+R12) + weight[n1][a2];
			weight[n1][n2] = weight[n2][n1] =  R12*Ran2/(Ran1+Ran2+R12);				
				
		/*	insert_to_map(a1, n1, weight[a1][n1]);
			insert_to_map(a2, n1, weight[a2][n1]);
			insert_to_map(n1, n1, weight[n1][n2]);
			erase_from_map(n2, a1);*/
		}
};
